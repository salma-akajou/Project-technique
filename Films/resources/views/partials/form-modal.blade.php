...
<div id="filmModal" class="fixed inset-0 z-[70] overflow-y-auto hidden">
    <div class="flex items-center justify-center min-h-screen p-4" data-action="modal-overlay">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg transform transition-all duration-300">
            <div class="flex justify-between items-center p-6 border-b border-gray-100">
                <h3 class="text-xl font-bold text-gray-800" id="filmModalTitle" data-title-add="{{ __('films.titles.add_film') }}" data-title-edit="{{ __('films.titles.edit_film') }}">
                    {{ __('films.titles.add_film') }}
                </h3>
                <button type="button" data-action="close-modal" class="text-gray-400 hover:text-gray-600 transition-colors">
                    <i data-lucide="x" class="size-5"></i>
                </button>
            </div>

            <form id="filmForm" action="{{ route('films.store') }}" data-store-action="{{ route('films.store') }}"
                  method="POST" enctype="multipart/form-data" class="p-6 space-y-4">
                @csrf
                <input type="hidden" name="_method" value="POST" id="filmFormMethod">
                <div>
                    <label class="block text-sm font-semibold mb-1.5">{{ __('films.fields.title') }}</label>
                    <input type="text" name="titre" id="filmTitre" class="py-2.5 px-4 block w-full border @error('titre') border-red-500 @else border-slate-500 @enderror rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500">
                    @error('titre')
                ...
                <div>
                    ...
                    <input type="text" name="directeur" id="filmDirecteur" class="py-2.5 px-4 block w-full border @error('directeur') border-red-500 @else border-slate-500 @enderror rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500">
                    @error('directeur')
                        <span class="text-red-500 text-xs mt-1">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea name="description" id="filmDescription" rows="3" class="py-2.5 px-4 block w-full border @error('description') border-red-500 @else border-slate-500 @enderror rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500"></textarea>
                    @error('description')
                        <span class="text-red-500 text-xs mt-1">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <div id="filmImagePreviewWrapper" class="mb-3 hidden">
                        <img id="filmImagePreview" src="" class="size-20 object-cover rounded-lg border border-gray-200">
                    </div>
                ...
                <div class="flex justify-end gap-x-3 pt-6 border-t border-gray-100 mt-6">
                    <button type="button" data-action="close-modal" class="py-2.5 px-6 text-sm font-semibold rounded-xl border border-gray-200 bg-white text-gray-800 hover:bg-gray-50 transition-colors">
