...
<nav class="flex items-center justify-center gap-x-1" aria-label="Pagination">
    ...
    @else
        <a href="{{ $paginator->previousPageUrl() }}" 
           data-action="paginate" data-page="{{ $paginator->currentPage() - 1 }}"
           class="min-h-9.5 min-w-9.5 py-2 px-2.5 inline-flex justify-center items-center gap-x-2 text-sm rounded-lg border border-transparent text-gray-800 hover:bg-gray-100 focus:outline-hidden focus:bg-gray-100" aria-label="Previous">
            <svg class="shrink-0 size-3.5" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                <path d="m15 18-6-6 6-6"></path>
            </svg>
            <span class="sr-only">Previous</span>
        ...
        @foreach ($elements as $element)
            ...
            @if (is_array($element))
                @foreach ($element as $page => $url)
                    ...
                    @else
                        <a href="{{ $url }}" 
                           data-action="paginate" data-page="{{ $page }}"
                           class="min-h-9.5 min-w-9.5 flex justify-center items-center border border-transparent text-gray-800 hover:bg-gray-100 py-2 px-3 text-sm rounded-lg focus:outline-hidden focus:bg-gray-100">{{ $page }}</a>
                    @endif
                @endforeach
            @endif
        @endforeach
    ...
    @if ($paginator->hasMorePages())
        <a href="{{ $paginator->nextPageUrl() }}" 
           data-action="paginate" data-page="{{ $paginator->currentPage() + 1 }}"
           class="min-h-9.5 min-w-9.5 py-2 px-2.5 inline-flex justify-center items-center gap-x-2 text-sm rounded-lg border border-transparent text-gray-800 hover:bg-gray-100 focus:outline-hidden focus:bg-gray-100" aria-label="Next">
